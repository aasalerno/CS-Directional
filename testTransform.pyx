...
cimport numpy as np
cimport numpy.fft as fft
from rwt import dwt, idwt
from rwt.wavelets import daubcqf

#DTYPE = np.int

def fft2c(np.array data_to_fft,axes=(-2,-1)):
    cdef np.array FFTdata = 1/np.sqrt(data_to_fft.size)*fft.fft2(data_to_fft,axes=axes)
    return FFTdata

def ifft2c(np.array data_to_ifft,axes=(-2,-1)):
    IFFTdata = np.sqrt(data_to_ifft.size)*fft.ifft2(data_to_ifft,axes=axes)
    return IFFTdata

def xfm(np.array data_to_xfm,scaling_factor = 4,L = 2):
    h = daubcqf(scaling_factor)[0]
    XFMdata = dwt(data_to_xfm,h,L)
    return XFMdata

def ixfm(np.array data_to_ixfm,scaling_factor = 4,L = 2):
    h = daubcqf(scaling_factor)[0]
    IXFMdata = idwt(data_to_ixfm,h,L)
    return IXFMdata

def TV(np.array data,strtag,ndir = 3):
