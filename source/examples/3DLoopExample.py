...
while True:
    try:
        pdf = samp.genPDF(N, P, pctg, radius=radius, cyl=np.hstack([1, N]), style='mult')
        break
    except:
        radius = 0.5*radius
...
if len(N) == 2:
    N = np.hstack([1, N])
    k = k.reshape(N)
if all(x == strtag[0] for x in strtag) and strtag[0] == 'spatial':
    arrays = [k for _ in range(N[0])]
    k = np.stack(arrays, axis=0)
