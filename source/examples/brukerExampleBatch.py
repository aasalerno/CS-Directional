    ...
    for TVWeight in [0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05]:
        for XFMWeight in [0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05]:
            ...
            for imdcs in ['zeros','ones','densCorr','imFull']:
                ...
                saveFig.save("brainData/exercise_irradiation/bruker_data/running_C/P14/rad_0.1/TV%s_XFM%s/%.2fper_result_im_dc_" % (float('%.1g' % TVWeight), float('%.1g' % XFMWeight) , pctg) + imdcs)
                np.save("brainData/exercise_irradiation/bruker_data/running_C/P14/rad_0.1/TV%s_XFM%s/%.2fper_result_im_dc_" % (float('%.1g' % TVWeight), float('%.1g' % XFMWeight) , pctg) + imdcs + ".npy",im_res)
