...
pdf = samp.genPDF(N[-2:], P, pctg, radius=radius, cyl=[1, N[-2], N[-1]], style='mult', pft=pft,ext=0.5)
k = d.dirPDFSamp(N,P=2,pctg=0.25,radius=0.2,dirs=dirs,cyl=True,taper=0.25)
...
data = k*dataFull
im_scan_wph = tf.ifft2c(data,ph=ph_ones,sz=szFull/N[0])
...
for j in range(nSteps+1):
    ...
    if locSteps[j]==0:
        ...
        im_FullSub = tf.ifft2c(np.fft.fftshift(dataFull,axes=(-2,-1)),ph=ph_onesSub,sz=szFull/N[0])
    else:
    ...
    im_dcSub = tf.ifft2c(dataSub / np.fft.ifftshift(pdfDiv,axes=(-2,-1)), ph=ph_scanSub, sz=szFull).real.copy()
    for i in xrange(N[0]):
    ...
    kMasked = (np.floor(1-kMasked)*pctgSamp[locSteps[j]]*1.0 + kMasked)
